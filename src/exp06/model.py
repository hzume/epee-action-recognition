"""Model module for exp06 with temporal consistency"""
...
class LitModel(L.LightningModule):
    """PyTorch Lightning wrapper with temporal consistency loss"""
    ...
    def __init__(
        ...
        self.prev_predictions = defaultdict(lambda: {"left": None, "right": None})
        
    def forward(self, x):
        return self.model(x)
    
    def compute_kl_loss(self, current_logits, prev_logits, temperature):
        """
        Compute KL divergence loss between current and previous predictions
        
        Args:
            current_logits: Current frame predictions
            prev_logits: Previous frame predictions
        ...
        log_probs_current = F.log_softmax(current_logits / temperature, dim=-1)
        
        # Apply temperature scaling and softmax to previous predictions
        probs_prev = F.softmax(prev_logits / temperature, dim=-1)
        
        # KL divergence: sum(p_prev * log(p_prev / p_current))
        kl_div = F.kl_div(log_probs_current, probs_prev, reduction='batchmean')
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_ce = (loss_left + loss_right) / 2
        
        # Temporal consistency loss
        kl_loss = torch.tensor(0.0, device=self.device)
        kl_count = 0
        
        for i in range(len(info["video_filename"])):
            video = info["video_filename"][i]
            frame_idx = info["frame_idx"][i].item()
            switched = info["switched"][i].item()
            
            # Create unique key for this video sequence
            key = f"{video}_{switched}_{frame_idx-1}"
            
            if key in self.prev_predictions:
                prev_left = self.prev_predictions[key]["left"]
                prev_right = self.prev_predictions[key]["right"]
            ...
            # Store current predictions for next frame
            current_key = f"{video}_{switched}_{frame_idx}"
            self.prev_predictions[current_key] = {
                "left": y_left_hat[i:i+1].detach(),
                "right": y_right_hat[i:i+1].detach()
            }
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # Loss (without temporal consistency for validation)
    ...
    def on_validation_epoch_start(self):
        ...
        # Clear prediction cache for validation
    ...
    def on_train_epoch_start(self):
        # Clear prediction cache at the start of each epoch
        self.prev_predictions.clear()
    ...
    def predict_step(self, batch, batch_idx):
        ...
        # Apply temporal smoothing during prediction
        smoothed_left = []
        smoothed_right = []
        ...
        for i in range(len(info["video_filename"])):
            ...
            # Current predictions
            curr_left = y_left_hat[i:i+1]
            curr_right = y_right_hat[i:i+1]
            ...
            if key in self.prev_predictions:
                ...
                if prev_left is not None:
                    # Smooth with previous predictions
                    alpha = 0.7  # Weight for current prediction
                    curr_left = alpha * curr_left + (1 - alpha) * prev_left
                    curr_right = alpha * curr_right + (1 - alpha) * prev_right
            
            smoothed_left.append(curr_left)
            smoothed_right.append(curr_right)
            
            # Store for next frame
            current_key = f"{video}_{switched}_{frame_idx}"
            self.prev_predictions[current_key] = {
                "left": curr_left.detach(),
                "right": curr_right.detach()
            }
        
        # Stack smoothed predictions
        y_left_hat = torch.cat(smoothed_left, dim=0)
        y_right_hat = torch.cat(smoothed_right, dim=0)
