"""Dataset module for exp07 with frame difference features"""
...
class EpeeDataset(Dataset):
    """Fencing action recognition dataset with frame difference features"""
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, Dict]:
        ...
        # Return additional info for temporal consistency
        info = {
    ...
    def preprocess(self, raw_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Add pose features first
        df = self._add_pose_features(df)
        
        # Add frame difference features before creating switched version
        df["switched"] = False
        df = self._add_frame_difference_features(df)
        df = df.drop(["left_bbox_area", "right_bbox_area"], axis=1)
        
        # Now create switched version after all features are added
    ...
    def _process_raw_df(self, raw_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Add keypoint columns (both normalized and absolute)
        for side in ["left", "right"]:
            for i in range(self.config.num_kp):
                data[f"{side}_keypoint_{i}_x"] = []
                data[f"{side}_keypoint_{i}_y"] = []
                # Store absolute coordinates for frame difference calculation
                data[f"{side}_keypoint_{i}_x_abs"] = []
                data[f"{side}_keypoint_{i}_y_abs"] = []
        ...
        # Normalize keypoints relative to bbox (keep absolute coordinates)
    ...
    def _add_frame_data(
        ...
        for i in range(self.config.num_kp):
            ...
            data[f"left_keypoint_{i}_x_abs"].append(left_row[f"keypoint_{i}_x"])
            data[f"left_keypoint_{i}_y_abs"].append(left_row[f"keypoint_{i}_y"])
            data[f"right_keypoint_{i}_x_abs"].append(right_row[f"keypoint_{i}_x"])
            data[f"right_keypoint_{i}_y_abs"].append(right_row[f"keypoint_{i}_y"])
    ...
    def _normalize_keypoints(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Drop unnecessary columns (but keep absolute keypoint coordinates)
        drop_cols = [
    ...
    def _add_frame_difference_features(self, df: pd.DataFrame) -> pd.DataFrame:
        """Add frame-to-frame difference features for keypoints"""
        # Sort by video and frame to ensure correct ordering
        df = df.sort_values(["video_filename", "frame_idx"]).reset_index(drop=True)
        
        # Initialize difference columns
        for side in ["left", "right"]:
            for i in range(self.config.num_kp):
                df[f"{side}_keypoint_{i}_dx"] = 0.0
                df[f"{side}_keypoint_{i}_dy"] = 0.0
        
        # Calculate differences for each video (only for non-switched data)
        for video, group_df in df.groupby("video_filename"):
            indices = group_df.index
            
            # Skip first frame (no previous frame)
            for idx in range(1, len(indices)):
                curr_idx = indices[idx]
                prev_idx = indices[idx - 1]
                
                # Calculate differences using absolute coordinates
                for side in ["left", "right"]:
                    for i in range(self.config.num_kp):
                        df.loc[curr_idx, f"{side}_keypoint_{i}_dx"] = (
                            df.loc[curr_idx, f"{side}_keypoint_{i}_x_abs"] - 
                            df.loc[prev_idx, f"{side}_keypoint_{i}_x_abs"]
                        )
                        df.loc[curr_idx, f"{side}_keypoint_{i}_dy"] = (
                            df.loc[curr_idx, f"{side}_keypoint_{i}_y_abs"] - 
                            df.loc[prev_idx, f"{side}_keypoint_{i}_y_abs"]
                        )
        
        # Normalize differences by bbox area (similar to keypoint normalization)
        # We need to reconstruct bbox area for normalization
        for idx, row in df.iterrows():
            # Normalize differences
            for i in range(self.config.num_kp):
                df.loc[idx, f"left_keypoint_{i}_dx"] /= df.loc[idx, "left_bbox_area"] ** 0.5
                df.loc[idx, f"left_keypoint_{i}_dy"] /= df.loc[idx, "left_bbox_area"] ** 0.5
                df.loc[idx, f"right_keypoint_{i}_dx"] /= df.loc[idx, "right_bbox_area"] ** 0.5
                df.loc[idx, f"right_keypoint_{i}_dy"] /= df.loc[idx, "right_bbox_area"] ** 0.5
        
        # Drop absolute coordinate columns as they're no longer needed
        abs_cols = [col for col in df.columns if col.endswith("_abs")]
        df = df.drop(columns=abs_cols)
    ...
    def _switch_side(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(self.config.num_kp):
            ...
            # Swap frame differences
            df_switched[f"left_keypoint_{i}_dx"] = -df[f"right_keypoint_{j}_dx"]
            df_switched[f"left_keypoint_{i}_dy"] = df[f"right_keypoint_{j}_dy"]
            df_switched[f"right_keypoint_{i}_dx"] = -df[f"left_keypoint_{j}_dx"]
            df_switched[f"right_keypoint_{i}_dy"] = df[f"left_keypoint_{j}_dy"]
